...
public class TranslationServices {
	...
	public Response translate(@PathParam("word") String wordToTranslate,
			@PathParam("language1") Language language1,
			@PathParam("language2") Language language2,
			@HeaderParam("token") String token) throws JSONException {
		User user = UsersCache.getUser(token);
		Set<Translation> wordTranslatedList = translate.translate(
				wordToTranslate, language1, language2, user);
	...
	public Response saveTranslation(@HeaderParam("token") String token,
			@HeaderParam("translation") Translation trans) throws JSONException {

		User user = UsersCache.getUser(token);
		UsersTranslations usersTranslations = new UsersTranslations(
				user.getEmail(), subtitleDAO.getSubtitleById(trans
						.getSubtitleDTOToTranslate().getId()),
				subtitleDAO.getSubtitleById(trans.getSubtitleDTOTranslated()
						.getId()));
		usersTranslationsDAO.save(usersTranslations);
	...
	public Response retrieveMyTranslations(@HeaderParam("token") String token,
			@HeaderParam("languageFrom") Language languageFrom,
			@HeaderParam("languageTo") Language languageTo)
			throws JSONException {

		User user = UsersCache.getUser(token);

		List<UsersTranslations> usersTranslations = usersTranslationsDAO
				.retrieveUsersTranslations(user.getEmail(), languageFrom,
						languageTo);
		List<Translation> translations = UserTranslationsMgr
				.convertUsersTranslationsToTranslation(usersTranslations);
	...
	public Response removeMyTranslation(@HeaderParam("token") String token,
		...
		usersTranslationsDAO.removeUsersTranslations(user.getEmail(),
				translation.getId());
		return Response.ok().header("Access-Control-Allow-Origin", "*").build();
	}
